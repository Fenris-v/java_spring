...
(function ($) {
    ...
    function addMessage(text) {
        window.scrollTo(0, 0);
        document.querySelectorAll('.actionMessage').forEach((message) => message.remove())
        let message = document.createElement('p');
        message.classList = "actionMessage"
        message.style.display = 'block';
        message.style.padding = '10px';
        message.style.background = '#6db33f';
        message.style.color = '#ffffff';
        message.innerText = text;
        document.querySelector('.Middle .wrap').prepend(message);
    }
    ...
    $(function () {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="' + book.image + '">' + '</a>' +
                    ...
                    (book.status === 'KEPT' ? ' <a class="Card-status" title="Отложена"><img src="/assets/img/icons/heart.svg" alt="Отложена"></a>' : '') + '</div>' + '<div class="Card-content">' + '<strong class="Card-title"><a href="#">' + book.title + '</a>' + '</strong>' + '<div class="Card-description">' + book.authors + '</div><div class="Card-cost">' +
                    (book.priceWithDiscount ? '<span class="Card-priceOld">₽' + book.price + '</span>' : '') + '<span class="Card-price">₽' + (book.priceWithDiscount ? book.priceWithDiscount : book.price) + '</span></div></div></div>';
            }
            return {
                init: function () {
                ...
                init: function () {
                    ...
                    $entryButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            var code = $('#code').val();
                            var mail = $('#mail').val();
                            $this.data('send', {code: code, contact: mail});
                            if (!error) {
                                postData('/auth', $this.data('send'), function (result) {
                                    console.log(result);
                                    if (result !== null && result.result) {
                ...
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                            ...
                            fetch('/books/changeBookStatus/' + $this.data("bookid"), {method: 'DELETE'})
                                .then(response => response.json())
                                .then((result) => result.success === true ? window.location.href = '/cart' : null);
                        } else if ($this.data('sendstatus') === 'KEPT') {
                            fetch('/books/postponed/' + $this.data("bookid"), {method: 'POST'})
                                .then(response => response.json())
                                .then((result) => addMessage('Книга добавлена в отложенные'));
                        } else if ($this.data('sendstatus') === 'UNKEPT') {
                            fetch('/books/postponed/' + $this.data("bookid"), {method: 'DELETE'})
                                .then(response => response.json())
                                .then((result) => result.success === true ? window.location.href = '/postponed' : null);
                        } else {
                            fetch('/books/changeBookStatus/' + $this.data("bookid"), {method: 'POST'})
                                .then(response => response.json())
                                .then((result) => addMessage('Книга добавлена в корзину'));
                        }
                    });
        ...
        var Rating = function () {
            return {
                init: function () {
                    $('.Rating_input input').on('click', function (e) {
                        let stars = document.querySelectorAll('.Rating_input .Rating-stars');
                        stars.forEach(el => el.classList.remove('Rating-star_view'));
                        for (let i = 0; i < e.target.value; i++) {
                            stars[i].classList.add('Rating-star_view')
                        }
                    });
                }
            };
    ...
    $('#popup__close, #overlay').click(function () {
        $('#popup').animate({opacity: 1}, 198, function () {
